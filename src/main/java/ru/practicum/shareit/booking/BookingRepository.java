...
public interface BookingRepository extends JpaRepository<Booking, Integer> {
  ...
  "LIMIT ?3 OFFSET ?2"
   ,nativeQuery = true)
    ...
    @Query(value = "select * " +
            ...
            "where booker_id=?1 " +
            "AND start_time >=now() " +
            "ORDER BY start_time DESC LIMIT ?3 OFFSET ?2 "
            , nativeQuery = true)
    ...
    @Query(value = "select * " +
            "from bookers " +
            "where booker_id=?1 " +
            "AND status LIKE 'WAITING' LIMIT ?3 OFFSET ?2 "
            , nativeQuery = true)
    ...
    @Query(value = "select *" +
            ...
            "now() between start_time and end_time LIMIT ?3 OFFSET ?2 "
            , nativeQuery = true)
    ...
    @Query(value = "select * " +
            "from bookers " +
            "where booker_id=?1 AND " +
            "  end_time<=now()    order by start_time DESC LIMIT ?3 OFFSET ?2 "
            , nativeQuery = true)
    ...
    @Query(value = "select * " +
            "from bookers " +
            "where booker_id=?1 " +
            "AND status LIKE 'REJECTED' LIMIT ?3 OFFSET ?2 "
            , nativeQuery = true)
 ...
 @Query(value = " select *  "+
    ...
    @Query(value = "select * " +
            ...
            "order by bookers.start_time DESC LIMIT ?3 OFFSET ?2 "
            , nativeQuery = true)
    ...
    @Query(value = "select * " +
            "FROM bookers inner join items i on i.id = bookers.item_id " +
            "where i.user_id=?1 " +
            "AND bookers.status LIKE 'WAITING' LIMIT ?3 OFFSET ?2"
            , nativeQuery = true)
    ...
    @Query(value = "select * " +
            "FROM bookers inner join items i on i.id = bookers.item_id " +
            "where i.user_id=?1 " +
            "AND bookers.status LIKE 'REJECTED' LIMIT ?3 OFFSET ?2 "
            , nativeQuery = true)
